  ...
  return (
    <div className="min-h-screen flex bg-neutral-100">
      ...
      <aside className={cn(
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-neutral-100">
          <div className="w-8 h-8 rounded-xl bg-gradient-brand flex items-center justify-center flex-shrink-0">
            <span className="text-white font-display font-bold text-sm select-none">PP</span>
          </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {NAV.map(item => (
            <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => cn(
              ...
              isActive
                ? 'bg-primary-50 text-primary-700 border-l-[3px] border-primary-500 pl-[9px]'
                : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900 border-l-[3px] border-transparent'
            )}>
              {item.icon}
              {item.label}
            </NavLink>
        ...
        <div className="p-3 border-t border-neutral-100">
          ...
          <button type="button" onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-danger-600 hover:bg-danger-50 transition-colors focus:outline-none focus-visible:shadow-focus">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-neutral-200/60 px-4 py-3 flex items-center gap-3 lg:px-6 sticky top-0 z-30 shadow-soft">
          ...
          <h1 className="text-sm font-display font-semibold text-neutral-900">{title || 'Admin Panel'}</h1>
        </header>
